...
def process_json2geojson_next_prev(
    i, k, data_out, data_err, data_out_next, data_out_prev
):
    ii = dict(
        {
            "type": "Feature",
    ...
    if cord_next and cord_next.get("lon"):
        ii_next = {
            "type": "Feature",
            "id": k,
            "properties": i,
...
@click.option(
    "--file", "-f", "in_file", required=True, help="Path to json file to be processed.",
)
...
def run_json2geojson_next_prev(in_file):
    ...
    with open(in_file, "r") as json_file:
        ...
        with click.progressbar(
            json_data, label="Process data", length=len(json_data)
        ) as bar:
            for k, i in enumerate(bar):
                process_json2geojson_next_prev(
                    i,
                    k,
                    data_out["features"],
                    data_err,
                    data_out_next["features"],
...
def run_group_8(in_file):
    ...
    with open(in_file, "r") as json_file:
        ...
        data_err_name = []
        with click.progressbar(
            json_data, label="Process data count ", length=len(json_data)
        ) as bar:
            for k, i in enumerate(bar):
                process_count(
                    i, data_out_count, data_err, data_out_names, data_err_name
                )

        # process data count

        data_out = [
            {"name": "2000>=", "count": 0, "escenes_count": 0, "scenes": []},
        ...
        with click.progressbar(
            data_out_count, label="Process data filter", length=len(data_out_count)
        ) as bar:
            for i in bar:
...
def run_goup_8_split(in_file, next_f, prev_f):
    ...
    with open(in_file, "r") as json_file:
        ...
        with click.progressbar(
            data_out_count, label="Process data mark", length=len(data_out_count)
        ) as bar:
            for i in bar:
        ...
        data_out = [
            {
                "name": "2000>=",
                "count": 0,
                "scenes": {"type": "FeatureCollection", "features": []},
            },
            {
                "name": "2000>1000>=",
                "count": 0,
        ...
        with click.progressbar(
            json_data, label="Process data filtered", length=len(json_data)
        ) as bar:
            for k, i in enumerate(bar):
                geojson = {
                    "type": "Feature",
        ...
        with click.progressbar(
            data_out, label="save data_out", length=len(data_out)
        ) as bar:
            for i in bar:
                count = i["count"]
                name = i["name"]
                new_name = in_file.replace(
                    ".json", f"_data_split_{name}-c{count}.geojson"
                )
                utils.save_json(new_name, i["scenes"])
        if next_f:
            with click.progressbar(
                data_out_next, label="save data_out_next", length=len(data_out_next)
            ) as bar:
                for i in bar:
                    count = i["count"]
                    name = i["name"]
                    new_name = in_file.replace(
                        ".json", f"_data_split_next_{name}-c{count}.geojson"
                    )
                    utils.save_json(new_name, i["scenes"])
        if prev_f:
            with click.progressbar(
                data_out_prev, label="save data_out_prev", length=len(data_out_prev)
            ) as bar:
                for i in bar:
                    count = i["count"]
                    name = i["name"]
                    new_name = in_file.replace(
                        ".json", f"_data_split_prev_{name}-c{count}.geojson"
                    )
                    utils.save_json(new_name, i["scenes"])

        with click.progressbar(
            data_err, label="save data_err", length=len(data_err)
        ) as bar:
            for i in bar:
                count = i["count"]
                name = i["name"]
                new_name = in_file.replace(
                    ".json", f"_data_split_err_{name}-c{count}.json"
                )
                utils.save_json(new_name, i["scenes"])

        with click.progressbar(
            data_err_gen, label="save error generic", length=1
        ) as bar:
            for i in bar:
