...
@click.option("--file", "-in", "in_file", required=True, help="Path to geojson.")
@click.option(
    "--field",
    "-f",
    "field",
        ...
        with click.progressbar(json_data["features"], label="Process data") as bar:
            ...
            for k, geo in enumerate(bar):
                ...
                for indx, field_t in enumerate(arr_fields):
                    geo["properties"][f"{field_t.strip()}"] = geo["properties"][field][
                        indx
                    ]
...
@click.option(
    "--file", "-in", "in_file", required=True, help="Path to geojson.",
)
@click.option(
    "--field",
    "-f",
    ...
    with open(in_file, "r") as json_file:
        ...
        with open(tiles_error, "r") as json_file_err:
            ...
            with click.progressbar(
                json_data["features"], label="Process errors"
            ) as bar:
                for geo in bar:
                    if geo["properties"][tile_field] in tiles_error_data:
            ...
            # verification
            geojson_err = [
                i for i in features if i["properties"].get("image_issue", False)
            ]
            ...
            if not len(geojson_err) == len(tiles_error_data):
                ...
                click.echo(
                    click.style(f"tiles_error_data: {tiles_error_data}", fg="green")
                )
